...
if (user.lives === 0) {
    ...
    p.textContent = `The good news is, it's just trivia. The bad news is, the whole point of this was trivia. I guess that's kind of the same thing, sorry I didn't come up with something more helpful.`;
    ...
    const p = document.createElement('p');
    p.textContent = `First off, I never doubted you. Secondly, I didn't think anybody would actually see this message, so...`;
    const img = document.createElement('img');
    ...
    const p = document.createElement('p');
    p.textContent = `You kind of gave it a good go, so that's something.`;
    const img = document.createElement('img');
